...
#ifndef execute_fixedDataVhK_HH_
#define execute_fixedDataVhK_HH_

#include <systemc>
using namespace std;
SC_MODULE(execute_fixedDataVhK) {
    static const unsigned int DATA_WIDTH = 1;
    static const unsigned int ADDR_WIDTH = 8;
    static const unsigned int execute_fixedDataVhK_depth = 129;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[execute_fixedDataVhK_depth];
    ...
    SC_CTOR(execute_fixedDataVhK) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < execute_fixedDataVhK_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (execute_fixedDataVhK_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < execute_fixedDataVhK_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (execute_fixedDataVhK_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > execute_fixedDataVhK_depth) {
...
#endif //execute_fixedDataVhK_HH_
