...
#ifndef V_read_Vj_idx_V_dncg_HH_
#define V_read_Vj_idx_V_dncg_HH_

#include <systemc>
using namespace std;
SC_MODULE(V_read_Vj_idx_V_dncg) {
    static const unsigned int DATA_WIDTH = 27;
    static const unsigned int ADDR_WIDTH = 11;
    static const unsigned int V_read_Vj_idx_V_dncg_depth = 1025;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[V_read_Vj_idx_V_dncg_depth];
    ...
    SC_CTOR(V_read_Vj_idx_V_dncg) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < V_read_Vj_idx_V_dncg_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (V_read_Vj_idx_V_dncg_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < V_read_Vj_idx_V_dncg_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (V_read_Vj_idx_V_dncg_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > V_read_Vj_idx_V_dncg_depth) {
...
#endif //V_read_Vj_idx_V_dncg_HH_
