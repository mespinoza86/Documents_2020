...
#ifndef execute_simConfigThq_HH_
#define execute_simConfigThq_HH_

#include <systemc>
using namespace std;
SC_MODULE(execute_simConfigThq) {
    static const unsigned int DATA_WIDTH = 27;
    static const unsigned int ADDR_WIDTH = 1;
    static const unsigned int execute_simConfigThq_depth = 2;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[execute_simConfigThq_depth];
    ...
    SC_CTOR(execute_simConfigThq) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < execute_simConfigThq_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (execute_simConfigThq_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < execute_simConfigThq_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (execute_simConfigThq_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > execute_simConfigThq_depth) {
...
#endif //execute_simConfigThq_HH_
