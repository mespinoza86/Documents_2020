...
#ifndef V_read_V_SIZE_chahbi_HH_
#define V_read_V_SIZE_chahbi_HH_

#include <systemc>
using namespace std;
SC_MODULE(V_read_V_SIZE_chahbi) {
    static const unsigned int DATA_WIDTH = 32;
    static const unsigned int ADDR_WIDTH = 1;
    static const unsigned int V_read_V_SIZE_chahbi_depth = 2;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[V_read_V_SIZE_chahbi_depth];
    ...
    SC_CTOR(V_read_V_SIZE_chahbi) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < V_read_V_SIZE_chahbi_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (V_read_V_SIZE_chahbi_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < V_read_V_SIZE_chahbi_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (V_read_V_SIZE_chahbi_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > V_read_V_SIZE_chahbi_depth) {
...
#endif //V_read_V_SIZE_chahbi_HH_
